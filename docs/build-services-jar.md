...
Take the stock, odexed `services.vdex` straight off the head unit and turn it into a working, sideload-enabled `services.jar` — entirely on your own machine. Pull the file over UART with the factory `su`, convert CompactDex to standard DEX with a pure-Python tool, patch four signature checks, rebuild.
...
- [Step 3 — Pull services.vdex Off the IHU](#step-3--pull-servicesvdex-off-the-ihu)
- [Step 4 — WSL Toolchain](#step-4--wsl-toolchain)
- [Step 5 — vdex → cdex](#step-5--vdex--cdex)
- [Step 6 — cdex → standard DEX](#step-6--cdex--standard-dex)
- [Step 7 — DEX → smali](#step-7--dex--smali)
- [Step 8 — Patch the 4 Changes](#step-8--patch-the-4-changes)
- [Step 9 — smali → jar](#step-9--smali--jar)
- [Step 10 — Verify Before Deploy](#step-10--verify-before-deploy)
- [Fix Fast](#fix-fast)
...
This guide builds the one file that unlocks APK sideloading on an ECarX E02 head unit — `services.jar` — **from the head unit's own firmware**.

> [!TIP]
> **What actually makes sideloading work:** Android's package installer blocks any APK whose signature does not match the system. Four small changes inside `services.jar` carry out and gate that check. **Make those four** and the installer stops rejecting foreign-signed APKs. Once you know which four, you can build the file yourself for any firmware version.
...
In Android, `compareSignatures` returning `0` means "signatures match". Force it to always return `0` and the installer stops rejecting unsigned or foreign-signed APKs. The other two are backups so nothing else re-blocks the install.

**Why this is harder than it sounds:**

Stock ECarX firmware ships `services.jar` as a 183-byte stub — the real code lives in `services.vdex` in Android's internal **CompactDex** (`cdex`) format. Standard tools (baksmali) cannot read CompactDex, and Google's `compact_dex_converter` needs a full AOSP build to compile. This guide uses a **pure-Python converter** instead — no AOSP, no untrusted binaries.
...
> [!WARNING]
> **Root without unlocking:** To read `services.vdex` off the IHU you only need **read** access as root. ECarX leaves a factory `su` at `/system/xbin/su` that hands root to the UART shell. That is all Step 3 uses.

> [!CAUTION]
> **Prerequisite:** This guide assumes you can already reach a UART shell on the IHU (CH340G at 3.3V, PuTTY at 921600). If not, [Step 2](#step-2--connect-uart) sets it up from scratch. Everything else you need is listed in [Step 1](#step-1--prerequisites).
...
| Heat shrink tube + heat gun | To cover the bare ends of the blue, green and red wires. Electrical tape works too — but they must not be left bare. [Step 2.1](#step-2--connect-uart). |
...
| ~2 GB free disk space | vdexExtractor source, the build toolchain, and the extracted smali tree. |
...
> [!CAUTION]
> **Before you buy anything else, get heat shrink or electrical tape.** The Micro JST GH cable arrives with six bare tinned ends and you only use three of them. The other three — blue, green and red — stay live next to your working wires, and red carries **3.3V from the IHU** whenever the unit is powered. Covering them is not optional. Full detail in [Step 2.1](#step-2--connect-uart).

### 1.2 — Software on the PC

| Software | Used for |
|---|---|
| PuTTY 64-bit | Serial terminal for UART, 921600 baud. This is how you pull the vdex in [Step 3](#step-3--pull-servicesvdex-off-the-ihu). Download `putty-64bit-*-installer.msi` from [the official PuTTY download page](https://www.chiark.greenend.org.uk/~sgtatham/putty/latest.html). |
| CH340G driver | So the COM port shows up in Device Manager. Get it from the chip maker, WCH: [CH341SER driver](https://www.wch-ic.com/downloads/CH341SER_EXE.html) — the same package covers the CH340/CH340G. |
| Ubuntu WSL | Runs the whole toolchain — see [Step 4](#step-4--wsl-toolchain). Nothing to download: `wsl --install` in [1.3](#13--install-wsl) pulls it in. Microsoft's reference is [learn.microsoft.com/windows/wsl/install](https://learn.microsoft.com/en-us/windows/wsl/install). |

### 1.3 — Install WSL

Run this in **PowerShell as Administrator**, then reboot. Skip it if `wsl -l -v` already lists an Ubuntu distro.
...
The actual build packages — `smali`, `baksmali`, `build-essential` and the rest — are installed inside WSL in [Step 4.1](#step-4--wsl-toolchain). Don't install them yet.

### 1.4 — Files you need

| File | Size | Where it comes from |
|---|---|---|
| `services.vdex` | ~20MB | Pulled off **your own** IHU over UART in [Step 3](#step-3--pull-servicesvdex-off-the-ihu). Must match the firmware on the unit you are patching. |
...
UART gives you a shell directly inside the IHU. That shell is how you pull `services.vdex` off the unit in [Step 3](#step-3--pull-servicesvdex-off-the-ihu).
...
> [!TIP]
> **For this guide you only need three wires:** yellow, black and white to the CH340G. Green and blue are only for BROM / device mode, which this guide never uses — leave the green pin un-shorted or the USB drive in [Step 3](#step-3--pull-servicesvdex-off-the-ihu) will not mount.

> [!CAUTION]
> **Insulate blue, green and red before you connect anything. This is the highest-risk part of the whole guide.** Those three wires are not used here, but they are still connected to the IHU and their ends are **bare tinned copper**. While the unit is powered:
>
...
> - **Green is USB mode.** Green touching red flips the USB port into device mode, and then the USB drive in [Step 3](#step-3--pull-servicesvdex-off-the-ihu) will not mount.
>
...
> [!WARNING]
> **Do not cut them off.** Blue, green and red are exactly the wires the [sideload guide](./sideload.md) shorts to enter BROM mode. Snip them and you throw away your recovery path — insulate, don't amputate.
...
- Make sure the IHU **POWER socket is unplugged** before wiring anything — that is **Socket Block A — Power**, the black connector sitting lowest in the ISO stack
...
- Set **Saved Sessions** to **UART**, then click **Save** to store the configuration
- Select the **UART** profile and click **Load**, or just double-click the **UART** profile
- Last look before power: blue, green and red still covered and tucked away from the three working wires
...
> [!TIP]
> **Tip:** Once the **UART** profile is saved you only need to double-click it next time. Right-click inside PuTTY pastes the clipboard — that is how you paste the commands in the following steps.
...
> [!TIP]
> **The prompt should look like this:** **Output**Expected prompt console:/ $ The `$` means you are **not** root yet. That is normal — [Step 3.1](#step-3--pull-servicesvdex-off-the-ihu) calls the factory `su` to fix that.

> [!WARNING]
> **No text at all in PuTTY?** TX and RX are probably swapped — swap those two wires and reconnect. Also re-check the COM port number, the Serial line, the cable orientation, the CH340 driver, and that the speed is exactly `921600`.

## Step 3 — Pull services.vdex Off the IHU

`UART · PuTTY`

The real framework code sits in `/system/framework/oat/arm64/services.vdex`. We copy it to a USB drive using the factory root shell — nothing is modified on the device.

### 3.1 — Get root with the factory su

In the PuTTY UART shell, the prompt starts as `$` (uid shell). Call the factory `su` by full path — do **not** type a bare `su`, that hits MagiskSU which is not set up here:
...
On a stock IHU the file is `services.vdex`. Confirm it is there and check the size:
...
Plug a FAT32 USB drive into the IHU, find its mount point, then copy:
...
> [!WARNING]
> **USB drive not visible?** If your unit is set up so the JST GH green pin is shorted (device mode), the USB port will not read a drive. Un-short it first, or transfer over ADB/WiFi instead. On a stock unit the port is in host mode by default, so the drive mounts normally. **Check the bare green wire is not touching red** — an accidental contact does exactly the same thing as a deliberate short ([Step 2.1](#step-2--connect-uart)).
...
Four tools: `baksmali`/`smali` (disassemble/assemble DEX), a compiler toolchain to build `vdexExtractor`, and the pure-Python `cdex2dex.py` converter.
...
On Ubuntu the `smali` package provides both `smali` and `baksmali` (v2.5.x, which handles Android 9 / API 28).

### 4.2 — Set up a working folder

The copy command below reaches into your Windows Desktop from inside WSL, so it needs your Windows username. List the accounts on the machine:
...
> [!TIP]
> If OneDrive sync is ON, your Desktop lives at `C:\Users\NAME\OneDrive\Desktop`. If it is OFF, it is at `C:\Users\NAME\Desktop`. Pick whichever one actually holds your files — get this wrong and the copy below will say "No such file or directory".
...
This is the tool that pulls the DEX out of the vdex and un-quickens the bytecode. Newer GCC treats one old warning as an error, so we disable `-Werror` before building.
...
> [!TIP]
> You want the binary at `~/vdexExtractor/bin/vdexExtractor` at the end. A few `-Wvla-parameter` warnings are fine — only errors would stop the build.

### 4.4 — Create the CompactDex → DEX converter

baksmali cannot read CompactDex, and Google's converter needs a full AOSP tree. This pure-Python tool does the conversion with no external dependencies. Paste the one-liner to write it out — it decodes into `~/svc/cdex2dex.py`:
...
> [!TIP]
> This writes a ~430-line Python script. It parses the CompactDex container, converts every code item to the standard DEX layout, rebuilds the string/type/method tables and the map list, and recomputes the checksum and SHA-1 signature. Debug info (line numbers, local names) is dropped — not needed for execution, and it avoids CompactDex's trickiest section.

## Step 5 — vdex → cdex

`WSL Ubuntu`

Extract the DEX from the vdex. vdexExtractor un-quickens the bytecode by default (turns ART's optimised opcodes back into standard ones), which is exactly what we need.
...
> [!TIP]
> **You should see:** **Output**Expected output [INFO] 1 Dex files have been extracted in total services.vdex_classes.cdex (~9.6M) The `.cdex` extension is the giveaway — it is still CompactDex, not standard DEX. That is what Step 6 fixes.
...
> [!TIP]
> **You should see:** **Output**Expected output [OK] wrote services.dex (9214036 bytes) magic dex 039 : True classes=4814 methods=55217 strings=95685 The numbers vary by firmware version. What matters is `magic dex 039 : True` — the output is now standard DEX.

> [!WARNING]
> **The converter is verified against ground truth:** for every method, the incoming-argument register count (`ins`) is fixed by the method signature. The converter's output matches the signature-derived value for all 55,217 methods, and no code item ends up with `registers < ins` — the exact error that would make Step 9 fail.

## Step 7 — DEX → smali

`WSL Ubuntu`

Disassemble the standard DEX into editable smali source.
...
> [!TIP]
> No errors and the `.smali` file listed means the conversion round-trip is sound. You now have the whole framework as smali under `smali_out/`.

## Step 8 — Patch the 4 Changes

`WSL Ubuntu`

Three of the changes replace a method body with a stub; the fourth rewrites a branch inside `installPackageLI`. In a 90,000-line file, hand-editing is error-prone, so use small patch scripts that find each spot and edit it.
...
`.locals 1` gives one local register (v0) on top of the parameters; `.locals 0` means none. smali recomputes the absolute register count automatically.
...
> [!TIP]
> **Expected — three PATCHED lines, and the stubs:** **Output**Expected output PATCHED: compareSignatures([Landroid/content/pm/Signature;[Landroid/content/pm/Signature;)I PATCHED: compareSignaturesWithAco([Landroid/content/pm/Signature;)I PATCHED: reconcileApps(Ljava/lang/String;)V .method public static compareSignatures(...)I .locals 1 const/4 v0, 0x0 return v0 .end method .method private reconcileApps(Ljava/lang/String;)V .locals 0 return-void .end method

> [!CAUTION]
> **If any line says NOT FOUND:** the method descriptor differs in your firmware version. Run `grep -n "compareSignatures" smali_out/com/android/server/pm/*.smali` and adjust the descriptor in the script to match.

### 8.4 — The fourth change: bypass the aco whitelist in installPackageLI

The three stubs above are not enough on their own. `installPackageLI` holds a second, separate check: a whitelist of package names, and any package not on it is rejected with `"signatures do not match the aco version; ignoring!"`. Sideloaded apps (Magisk, etc.) are never on that list, so this check must be neutralised too. This script redirects all three branch paths of that check to the safe continuation label, making the reject block unreachable.
...
> [!WARNING]
> Label names (`:cond_XXXX`) vary by firmware — the script finds them automatically, so it works on any ECarX E02 Android 9 build. If it prints `guard redirects: 0`, stop and re-check before deploying.

## Step 9 — smali → jar

`WSL Ubuntu`

Reassemble the patched smali into a DEX, then package it into `services.jar`.
...
> [!WARNING]
> **The `-a 28` is not optional.** Without it, smali defaults to `dex 035`. Android 9 loads a 035 framework jar and boots, but runs it under older rules and your patches never take effect — installs still fail with the aco error. `-a 28` (API 28) produces `dex 039`, which the framework expects. Confirm with `head -c 8 classes.dex | xxd` — it must read `dex 039`.

> [!CAUTION]
> **If smali says "requires at least N registers":** that is the CompactDex register-decode bug the converter was written to avoid. Make sure you are on the verified `cdex2dex.py` from Step 4.4 — re-run from Step 6 with it.

## Step 10 — Verify Before Deploy

`WSL Ubuntu`

A wrong `services.jar` can bootloop the IHU, so confirm the three stubs are in the final DEX before deploying.
...
> [!TIP]
> The first command must show `6465 780a 3033 3900` = `dex 039`. If it ends `3033 3500` (035), re-run Step 9 with `-a 28`. The aco string still exists in the code, but after Step 8.4 nothing branches into it.

### 10.1 — Copy the finished jar to your Desktop

The build lands at `~/svc/services.jar` inside WSL. Copy it out to your Desktop so it is easy to find and drop onto a USB drive:
...
> [!TIP]
> It is already named `services.jar` — the exact name the framework expects. Keep that name when you place it in the pendrive `mod` folder and when you copy it into `/system/framework/`.

> [!WARNING]
> **Deploy:** This `services.jar` drops into the [sideload guide](./sideload.md) — stage it to `/data/local/tmp/`, swap it into `/system/framework/`, delete the stale `oat` cache, reboot. Keep the original `.orig` files as your recovery path.
